...
class BraTS2019(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        ...
        train_path_change = self._base_dir + '/train_change.txt'
        test_path = self._base_dir+'/val.txt'

        if split == 'train':
            with open(train_path, 'r') as f:
                self.image_list = f.readlines()
        if split == 'train_change':
            with open(train_path_change, 'r') as f:
                self.image_list = f.readlines()
        elif split == 'test':
            with open(test_path, 'r') as f:
                self.image_list = f.readlines()
...
class BraTS2019_kfold(Dataset):
    """ BraTS2019 Dataset """

    def __init__(self, base_dir=None, split='train', num=None, transform=None,fold=None):
        self._base_dir = base_dir
        self.transform = transform
        self.sample_list = []
        self.fold = fold

        train_path = self._base_dir+'/fold{}/train.txt'.format(self.fold)
        test_path = self._base_dir+'/fold{}/test.txt'.format(self.fold)
