...
def var_all_case(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA", fold=None):
    if dataset_name == "LA":
    ...
    elif dataset_name == "Brats19":
        base_dir = '/data/sohui/BraTS/data/BraTs2019'
        with open(base_dir + '/{}'.format('val.txt'), 'r') as f:
            image_list = f.readlines()
        image_list = [base_dir + "/" + item.replace('\n', '') + ".h5" for item in image_list]
    elif dataset_name == "Brats19_kfold":
        base_dir = '/data/sohui/BraTS/data/BraTs2019'
        with open(base_dir + '/fold{}/test.txt'.format(fold), 'r') as f:
            image_list = f.readlines()
        image_list = [base_dir + "/" + item.replace('\n', '') for item in image_list]
        print('/fold{}/test.txt'.format(fold))
