...
class BraTS2021(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = nib.load(self._base_dir + "/{}/{}_flair.nii.gz".format(image_name, image_name))
        image = image.get_fdata()
        label = nib.load(self._base_dir + "/{}/{}_seg.nii.gz".format(image_name, image_name))
        label = label.get_fdata()
        #label_= np.zeros_like(label)
        label= np.where(label == 4,3,label)

        #label_ = np.place(label,label>=4,3)
        ...
        # for idx in range(3):
        plt.subplot(2, 1, 1)
        plt.imshow(image[:,:,100:101])
        plt.subplot(2, 1, 2)
        plt.imshow(label[:,:,100:101])
