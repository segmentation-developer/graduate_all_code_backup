...
from networks.vnet_2task_SDM_foreOnly import VNet
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs_2class, outputs_1class_sig = model(volume_batch)  ## model = student
            outputs_soft_2class = torch.softmax(outputs_2class, dim=1)

            with torch.no_grad():
                gt_dis = compute_sdf_foreOnly(label_batch[:].cpu(
                ).numpy(), outputs_1class_sig[:labeled_bs, 0, ...].shape)
            ...
            preds = preds.reshape(T, stride, 2, d, w, h)  # 8,2,2,d,w,h
            seg_avg = torch.mean(preds, dim=0)  # 2,2,d,w,h
            ...
            loss_sdf = mse_loss(outputs_1class_sig[:labeled_bs, 0, ...], gt_dis)
            ...
            consistency_1class_loss = mse_loss(outputs_1class_sig[labeled_bs:], SDM_avg)  ## (2,1,96,96,96)

            #consistency_dice = 0.5* (dice_loss(outputs_soft_2class[labeled_bs:], seg_avg, target_one_hot_encoder=False))
