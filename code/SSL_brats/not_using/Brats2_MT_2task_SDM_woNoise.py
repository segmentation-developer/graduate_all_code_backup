...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            consistency_dist = torch.mean((
                outputs_soft_2class[labeled_bs:]- torch.softmax(ema_output,dim=1))**2) # (batch, 2, 112,112,80)


            consistency_1class_loss = mse_loss(outputs_1class_tanh[labeled_bs:], ema_output_1class_tanh)       ## (2,1,96,96,96)

            loss = supervised_loss + consistency_weight * (consistency_dist + consistency_1class_loss)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            wandb.log({
                ...
                "consistency_loss" : consistency_dist.item(),
