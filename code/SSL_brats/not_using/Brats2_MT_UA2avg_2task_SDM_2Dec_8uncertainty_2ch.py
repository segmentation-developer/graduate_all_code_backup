...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            T = 8
            bs, ch, d, w, h = unlabeled_volume_batch.shape
            ...
            preds = preds.reshape(T, stride, 2, d, w, h)  # 8,2,2,w,h,d
            # seg_avg = torch.mean(preds, dim=0)  # bs,ch,w,h,d
            '''
            ## 2개의 ch 합해서 uncertainty 구하기
            uncertainty = -1.0 * torch.sum(preds * torch.log(preds + 1e-6), dim=2, keepdim=True) # 8,2,1(ch),w,h,d
            uncertainty_min = torch.argmin(uncertainty, axis=0) # 2,1,w,h,d
            uncertainty_M = torch.zeros(T, bs, ch, d,w,h).cuda() # 8,bs,1,w,h,d
            for i in range(T) : # 0~7
                uncertainty_M[i] = torch.where(uncertainty_min == i ,1 , 0 )
            uncertainty_seg = uncertainty_M * preds    # 8,2,1,w,h,d * 8,2,2,w,h,d
            uncertainty_seg = torch.sum(uncertainty_seg, dim = 0)   # 2,2,w,h,d
            '''

            ## 각 ch 에 대한 uncertainty 구하기 (8번 중)  ->  2ch에 대한 uncertainty weight 구해서 곱하기 
            uncertainty = -1.0 * preds * torch.log(preds + 1e-6)  # 8,2,2(ch),w,h,d
            uncertainty_min = torch.argmin(uncertainty, axis=0)  # bs,2,w,h,d
            uncertainty_M = torch.zeros(T, bs, args.num_classes, d, w, h).cuda()  # 8,bs,2,w,h,d
            for i in range(T):  # 0~7
                uncertainty_M[i] = torch.where(uncertainty_min == i, 1, 0)
            uncertainty_seg = uncertainty_M * preds  # 8,2,2,w,h,d * 8,2,2,w,h,d
            uncertainty_seg = torch.sum(uncertainty_seg, dim=0)  # 2,2,w,h,d
            uncertainty_mean = torch.mean(uncertainty)
            '''
            second_uncertainty = -1.0 * uncertainty_seg * torch.log(uncertainty_seg + 1e-6)  # 2,2,w,h,d
            second_uncertainty_w = torch.softmax(1 - second_uncertainty, dim=1)   # 2,2,w,h,d
            uncertainty_seg  = (2 * second_uncertainty_w) * uncertainty_seg        ## ch별 weight 편차가 너무 커지는 거 아닌가?
            '''
            ...
            supervised_loss = 0.5 * (loss_dice + loss_ce) + loss_sdf

            #####################################################################################################unsupervised loss
            consistency_weight = get_current_consistency_weight(iter_num // 150)
            consistency_mse = torch.mean((outputs_soft_2class[labeled_bs:] - uncertainty_seg) ** 2)  # (batch, 2, 112,112,80)

            consistency_1class_loss = mse_loss(outputs_1class_tanh[labeled_bs:], SDM_avg)  ## (2,1,96,96,96)
            ...
            wandb.log({
                ...
                #"consistency_segDice_loss": consistency_dice.item(),
                "uncertainty_mean": uncertainty_mean.item()
