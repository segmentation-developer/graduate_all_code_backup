...
from networks.vnet_2task_SDMreverse import VNet
from utils import losses, metrics, ramps
from brats_val_3D_2task import test_all_case
from utils.util import compute_sdf_reverse
import wandb
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                gt_dis = compute_sdf_reverse(label_batch[:].cpu().numpy(), outputs_1class_tanh[:labeled_bs, 0, ...].shape)
                #gt_dis[ gt_dis > 0] = 1     ## inside :+1, background : -1 ~
