...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            uncertainty = -1.0 * seg_avg * torch.log(seg_avg + 1e-6)  # 2,2,w,h,d
            uncertainty_mean = torch.mean(uncertainty)
            uncertainty_w = 2* (torch.softmax(1 - uncertainty, dim=1)) # 2,2,w,h,d
            ...
            consistency_mse = torch.mean(uncertainty_w*((outputs_soft_2class[labeled_bs:] - seg_avg) ** 2) ) # (batch, 2, 112,112,80)
