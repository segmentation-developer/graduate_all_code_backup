...
def test_all_case(net, base_dir, test_list="full_test.list", num_classes=4, patch_size=(48, 160, 160), stride_xy=32, stride_z=24, fold=None):
    with open(base_dir + '/fold{}/{}'.format(fold, test_list), 'r') as f:
        image_list = f.readlines()
    image_list = [base_dir + "/" + item.replace('\n', '') for item in image_list]
    print('fold{}/{}'.format(fold, test_list))
    ...
    for image_path in tqdm(image_list):
        ...
        if np.sum(prediction) == 0:
            ...
            single_metric = cal_metric(prediction, label[:])
