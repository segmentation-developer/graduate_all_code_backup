...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            T = args.T
            b, _, d, w, h = volume_batch.shape
            volume_batch_r = volume_batch
            preds = torch.zeros([b*T, 2, d, w, h]).cuda()
            SDM_preds = torch.zeros([b*T, 1, d, w, h]).cuda()
            for i in range(T):
                ema_inputs = volume_batch_r + torch.clamp(torch.randn_like(volume_batch_r) * 0.1, -0.2, 0.2)
                with torch.no_grad():
                    preds[b*i:b*(i + 1)], SDM_preds[b* i:b* (i + 1)] = ema_model(ema_inputs)  ## unlabel의 shape를 *2로 복사한 값에 새로운 noise를 >계속 넣어줘서 teacher model에 계속 실험  -> preds에 계속 결과 쌓기
            preds = torch.softmax(preds, dim=1)
            preds = preds.reshape(T, b, 2, d, w, h)  # 8,bs,ch,w,h,d
            seg_avg = torch.mean(preds, dim=0)  # 2,2,w,h,d

            SDM_preds = SDM_preds.reshape(T, b, 1, d, w, h)
            ...
            consistency_mse = torch.mean((outputs_soft_2class - seg_avg) ** 2)  # (batch, 2, 112,112,80)

            consistency_1class_loss = mse_loss(outputs_1class_tanh, SDM_avg)  ## (2,1,96,96,96), SDM loss가 80배정도 큼
