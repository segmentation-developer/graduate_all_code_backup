...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            ## teacher model
            with torch.no_grad():
                ema_seg, ema_sdm = ema_model(ema_inputs)
                ema_seg_soft = torch.softmax(ema_seg, dim=1)
            ...
            consistency_mse = torch.mean((outputs_soft_2class[labeled_bs:] - ema_seg_soft) ** 2)  # (batch, 2, 112,112,80)

            consistency_1class_loss = mse_loss(outputs_1class_tanh[labeled_bs:], ema_sdm)  ## (2,1,96,96,96), SDM loss가 80배정도 큼
