...
from brats_val_3D_2task_ConfirmUncertainty import test_all_case
...
parser.add_argument('--labeled_num', type=int, default=25,
                    help='labeled data')
parser.add_argument('--total_data_num', type=int, default=250,
...
parser.add_argument('--tau', type=float,  default=0.4, help='sharpening Predictions temperature')
...
def train(args, snapshot_path):
    ...
    unlabeled_idxs = list(range(args.labeled_num, args.total_data_num))
    ...
    total_data_len = args.total_data_num
    len_10p_data = int(total_data_len * 0.1)        # 25
    total_uncertainty = torch.zeros(max_iterations).cuda()
    iter_num = 0
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            uncertainty_student = -1.0 * torch.sum(outputs_soft_2class * torch.log(outputs_soft_2class + 1e-6), dim=1, keepdim=True)
            uncertainty_student_label = torch.mean(uncertainty_student[:labeled_bs])
            uncertainty_student_unlabel = torch.mean(uncertainty_student[labeled_bs:])
            ...
            uncertainty_teacher_unlabel = torch.mean(-1.0 * torch.sum(seg_avg * torch.log(seg_avg + 1e-6), dim=1))
            total_uncertainty[iter_num] = uncertainty_teacher_unlabel

            ## uncertainty threshold  -> sharpening function  -> seg_avg 생성
            use_sharpening = 0
            if iter_num >= (args.max_iterations*0.05):
                if torch.mean(total_uncertainty[(iter_num-len_10p_data):iter_num]) > uncertainty_teacher_unlabel : ##확실한 pred
                    use_sharpening = 1                  ## sharpening을 얼마나 이용하는지 그래프로 확인하기 위한 변수
                    preds = preds_copy / args.tau
                    preds = torch.softmax(preds, dim=1)
                    preds = preds.reshape(T, stride, 2, d, w, h)  # 8,bs,ch,w,h,d
                    seg_avg = torch.mean(preds, dim=0)  # 2,2,w,h,d
            ...
            wandb.log({
                ...
                "uncertainty_teacher_unlabel": uncertainty_teacher_unlabel,
                "uncertainty_student_label": uncertainty_student_label,
                "uncertainty_student_unlabel":uncertainty_student_unlabel,
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                logging.info(
                    'iteration %d : dice_score : %f hd95 : %f uncertainty : %f' % (iter_num, avg_metric[0, 0].mean(), avg_metric[0, 1].mean(),uncertainty_val))
                wandb.log({
                    "uncertainty_validation": uncertainty_val
                })
...
if __name__ == "__main__":
    ...
    snapshot_path = '/data/sohui/BraTS/data/brats_2class_train_result/BraTs19_label_{}_{}/{}/{}_{}_T={}'\
        .format(args.labeled_num, args.total_data_num,args.exp,args.model,args.max_iterations,args.T)
