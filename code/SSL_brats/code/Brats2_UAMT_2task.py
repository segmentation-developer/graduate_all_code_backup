...
from networks.vnet_2task import VNet
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs_2class,outputs_1class = model(volume_batch)                       ## model = student
            outputs_soft_2class = torch.softmax(outputs_2class, dim=1)
            outputs_soft_1class = torch.sigmoid(outputs_1class)

            with torch.no_grad():
                ema_output,ema_output_1class = ema_model(ema_inputs)              ##ema model = teacher
            ...
            for i in range(T // 2):
                ...
                with torch.no_grad():
                    preds[2 * stride * i:2 * stride * (i + 1)],__ = ema_model(ema_inputs)  ## unlabel의 shape를 *2로 복사한 값에 새로운 noise를 계속 넣어줘서 teacher model에 계속 실험  -> preds에 계속 결과 쌓기
            preds = torch.softmax(preds, dim=1)
            preds = preds.reshape(T, stride, 2, d, w, h)
            preds = torch.mean(preds, dim=0)
            uncertainty = -1.0 * torch.sum(preds * torch.log(preds + 1e-6), dim=1,
                                           keepdim=True)  ##entropy 가 낮으면 확실, uncertainty 값이 낮아지길 원함
            ...
            loss_mse =  mse_loss(outputs_soft_1class[:labeled_bs],label_batch[:labeled_bs].unsqueeze(1).float())

            supervised_loss = 0.5 * (loss_dice + loss_ce) + loss_mse

            #####################################################################################################unsupervised loss
            consistency_weight = get_current_consistency_weight(iter_num//150)
            consistency_dist = losses.softmax_mse_loss(
                outputs_2class[labeled_bs:], ema_output)  # (batch, 2, 112,112,80)
            threshold = (0.75+0.25*ramps.sigmoid_rampup(iter_num,max_iterations))*np.log(2)
            mask = (uncertainty < threshold).float()  ## ucertainty 가 낮은 부분만 loss 반영
            consistency_loss = torch.sum(mask * consistency_dist) / (2 * torch.sum(mask) + 1e-16)

            consistency_1class_loss = mse_loss(outputs_soft_1class[labeled_bs:], torch.sigmoid(ema_output_1class))       ## (2,1,96,96,96)

            loss = supervised_loss + consistency_weight * (consistency_loss + consistency_1class_loss)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
