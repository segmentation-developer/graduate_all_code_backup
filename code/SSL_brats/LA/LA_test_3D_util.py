...
def test_all_case(net, base_dir, method="unet_3D", test_list="test_list.txt", num_classes=4, patch_size=(48, 160, 160), stride_xy=32, stride_z=24, save_result=True, test_save_path=None, metric_detail=0 , nms=1):
    ...
    ## variance estimation
    var_dice, var_jacc, var_hd, var_asd = [], [], [], []
    var_4metric = []
    for i in range(len(metric_all)):
        var_dice.append(metric_all[i][0])
        var_jacc.append(metric_all[i][1])
        var_hd.append(metric_all[i][2])
        var_asd.append(metric_all[i][3])
    var_d = np.std(var_dice)
    var_j = np.std(var_jacc)
    var_h = np.std(var_hd)
    var_a = np.std(var_asd)
    var_4metric.append([var_d, var_j, var_h, var_a])

    f.close()
    print("Testing end")

    return total_metric / len(image_list), var_4metric
