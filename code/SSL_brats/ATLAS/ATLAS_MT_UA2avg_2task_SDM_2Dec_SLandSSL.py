...
parser.add_argument('--patch_size', type=list,  default=[128,128,128], ## brats_2class max_size : 148,188,156
...
parser.add_argument('--gpu', type=str,  default='4,7', help='GPU to use')
parser.add_argument('--T', type=int,  default=8, help='ATD iter')
parser.add_argument('--sl_max_iterations', type=int,  default=10000, help='sl max iter')
...
def train(args, snapshot_path):
    ...
    logging.info("{} iterations per epoch".format(len(unlabeled_data)))

    ##supervised
    sl_max_iterations = args.sl_max_iterations
    iter_num = 0
    max_epoch = sl_max_iterations // len(trainloader_label) + 1
    best_performance = 0.0
    iterator = tqdm(range(max_epoch), ncols=70)

    for sl_epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader_label):
            ...
            loss_ce = ce_loss(outputs_2class, label_batch)
            loss_dice = dice_loss(outputs_soft_2class, label_batch.unsqueeze(1).float())
            loss_sdf = mse_loss(outputs_1class_tanh[:, 0, ...], gt_dis)
            ...
            lr_ = base_lr * (1.0 - iter_num / sl_max_iterations) ** 0.9
            ...
            if iter_num > 0 and iter_num % 500 == 0:
                ...
                if avg_metric[:, 0].mean() > best_performance:
                    ...
                    save_mode_path = os.path.join(snapshot_path,
                                                  'sl_iter_{}_dice_{}.pth'.format(
                                                      iter_num, round(best_performance, 4)))
                    save_best = os.path.join(snapshot_path,
                                             'sl_{}_best_model.pth'.format(args.model))
                    torch.save(model.state_dict(), save_mode_path)
                    torch.save(model.state_dict(), save_best)

                writer.add_scalar('info/sl_val_dice_score',
                                  avg_metric[0, 0], iter_num)
                writer.add_scalar('info/sl_val_hd95',
                                  avg_metric[0, 1], iter_num)
                logging.info(
                    'sl_iteration %d : dice_score : %f hd95 : %f' % (iter_num, avg_metric[0, 0].mean(), avg_metric[0, 1].mean()))
                model.train()

            if iter_num % 2000 == 0:
                save_mode_path = os.path.join(
                    snapshot_path, 'sl_iter_' + str(iter_num) + '.pth')
                torch.save(model.state_dict(), save_mode_path)
                logging.info("save model to {}".format(save_mode_path))

            if iter_num >= sl_max_iterations:
                break
        if iter_num >= sl_max_iterations:
            iterator.close()
            break
...
if __name__ == "__main__":
    ...
    total_iter = args.max_iterations + args.sl_max_iterations
    snapshot_path = '/data/sohui/ATLAS/ATLAS_2class_train_result/ATLAS_label_{}_{}/{}/{}_{}_T={}'.format(args.labeled_num, args.total_labeled_num,args.exp,args.model,total_iter,args.T)
