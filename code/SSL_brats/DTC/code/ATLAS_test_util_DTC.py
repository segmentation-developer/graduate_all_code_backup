...
def test_all_case(root_path,net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=0, nms=0):
    total_metric = 0.0
    loader = tqdm(image_list) if not metric_detail else image_list
    ith = 0
    for image_path in loader:
        # id = image_path.split('/')[-2]
        image = nib.load(root_path + "/images" + "/{}".format(image_path))
        label = nib.load(root_path + "/labels" + "/{}".format(image_path))
