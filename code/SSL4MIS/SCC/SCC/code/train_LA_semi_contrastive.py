...
parser.add_argument('--root_path', type=str, default='/data/sohui/LA_dataset/2018LA_Seg_TrainingSet/', help='data path')
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if args.has_contrastive == 1:
                ...
                create_center_1_bg = center_model(outputs_1[:,0,...].unsqueeze(1))# 4,1,x,y,z->4,2    ch0
                create_center_1_la = center_model(outputs_1[:,1,...].unsqueeze(1))  # ch1
                ...
                lb_center_12_bg = torch.cat((create_center_soft_1_bg[:labeled_bs,...], create_center_soft_2_bg[:labeled_bs,...]),dim=0)# 4,2     super_dec1,2  background
                lb_center_12_la = torch.cat((create_center_soft_1_la[:labeled_bs,...], create_center_soft_2_la[:labeled_bs,...]),dim=0)     #super_dec1,2 label
                un_center_12_bg = torch.cat((create_center_soft_1_bg[labeled_bs:,...], create_center_soft_2_bg[labeled_bs:,...]),dim=0)     #unsuper_dec1,2  background
                un_center_12_la = torch.cat((create_center_soft_1_la[labeled_bs:,...], create_center_soft_2_la[labeled_bs:,...]),dim=0)     #unsuper_dec1,2 label
