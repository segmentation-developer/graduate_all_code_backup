...
parser.add_argument('--exp', type=str,
                    default='HV_result/Unet2_dsv1ce', help='experiment_name')
...
parser.add_argument('--patch_size', type=list,  default=[176,176,48],
...
def train(args, snapshot_path):
    ...
    train_transforms = Compose(
        [
            ...
            SpatialPadd(keys=["image", "label"], spatial_size=(192,192,48), mode="constant"),  #d: 48~144
            RandCropByPosNegLabeld(
                keys=["image", "label"],
                label_key="label",
    ...
    SL_trainloader = DataLoader(db_train_SL, batch_size=args.labeled_bs,shuffle=False,
                             num_workers=4, pin_memory=True, worker_init_fn=worker_init_fn)
    UL_trainloader = DataLoader(db_train_UL, batch_size=args.labeled_bs,shuffle=False,
                                num_workers=4, pin_memory=True, worker_init_fn=worker_init_fn)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(zip(SL,UL)):       #0,1 : SL_trainloader(bs:1), UL_trainloader(bs:1)
            sl_volume_batch, sl_label_batch = sampled_batch[0]['image'].cuda(), sampled_batch[0]['label'].cuda()
            ul_volume_batch, ul_label_batch = sampled_batch[1]['image'].cuda(), sampled_batch[1]['label'].cuda()

            volume_batch = torch.cat((sl_volume_batch,ul_volume_batch),0)
            label_batch = torch.cat((sl_label_batch, ul_label_batch), 0)
            ...
            if iter_num == 1:
                ...
                # for idx in range(3):
                plt.subplot(6, 1, 1)
                plt.imshow(imgAddBound1[0][:, :, 60:61].detach().cpu())
                plt.subplot(6, 1, 2)
                plt.imshow(imgAddBound2[0][:, :, 60:61].detach().cpu())
                plt.subplot(6, 1, 3)
                plt.imshow(gtAddBound[0][:, :, 60:61].detach().cpu())
                plt.subplot(6, 1, 4)
                plt.imshow(label_batch[0][:, :, 60:61].detach().cpu())
                plt.subplot(6, 1, 5)
                plt.imshow(boundary_label[0][:, :, 60:61].detach().cpu())
                plt.subplot(6, 1, 6)
                plt.imshow(dsv1_F_soft[0][1][:, :, 60:61].detach().cpu())
            ...
            dsv1_F = model1(volume_batch + noise )
            ...
            noise = torch.clamp(torch.randn_like(volume_batch) * 0.1, -0.2, 0.2)
            dsv1_S = model2(volume_batch + noise )
            ...
            ##supervised :dice CE
            loss1 = (ce_loss(dsv1_F[:args.labeled_bs], label_batch[:args.labeled_bs].squeeze(1).long()) + dice_loss(
                dsv1_F_soft[:args.labeled_bs], label_batch[:args.labeled_bs]))
            loss2 = (ce_loss(dsv1_S[:args.labeled_bs], label_batch[:args.labeled_bs].squeeze(1).long()) + dice_loss(
                dsv1_S_soft[:args.labeled_bs], label_batch[:args.labeled_bs]))
            ...
            pseudo_outputs1 = torch.argmax(
                dsv1_F[args.labeled_bs:].detach(), dim=1, keepdim=False)
            # pseudo_outputs1 = class_create(pseudo_outputs1,num_classes)

            pseudo_outputs2 = torch.argmax(
                dsv1_S[args.labeled_bs:].detach(), dim=1, keepdim=False)


            pseudo_supervision1 = ce_loss(dsv1_F[args.labeled_bs:], pseudo_outputs2)
            pseudo_supervision2 = ce_loss(dsv1_S[args.labeled_bs:], pseudo_outputs1)
            ...
            wandb.log({
                ...
                "pseudo_supervision1":pseudo_supervision1.item(),
                "pseudo_supervision2": pseudo_supervision2.item(),
                ...
                #"SL_boundary_loss": boundary_loss.item()
                #"consistency_loss_aux5": consistency_loss_aux5.item(),

            })

            logging.info('iteration %d : supervised_loss : %f  unsupervised_loss : %f'  % (
            ...
            if iter_num > 0 and iter_num % 2000 == 0:
                ...
                avg_metric1 = test_all_case(
                    model1, val_loader=val_loader, num_classes=num_classes, patch_size=args.patch_size,
                ...
                writer.add_scalar('info/model1_val_hd95',
                                  avg_metric1[:, 1].mean(),  iter_num)
                logging.info(
                    'iteration %d : model1_dice_score : %f model1_class1_dice: %f model1_class2_dice: %f  model1_hd95 : %f  model1_class1_hd95 : %f  model1_class2_hd95 : %f' % (
                        iter_num, avg_metric1[:, 0].mean(), avg_metric1[0, 0], avg_metric1[1, 0],\
                        avg_metric1[:, 1].mean(), avg_metric1[0, 1], avg_metric1[1,1]))
                model1.train()

                model2.eval()
                avg_metric2 = test_all_case(
                    model2, val_loader=val_loader, num_classes=num_classes, patch_size=args.patch_size,
                    stride_xy=32, stride_z=32)
                ...
                writer.add_scalar('info/model2_val_hd95',
                                  avg_metric2[:, 1].mean(),  iter_num)
                logging.info(
                    'iteration %d : model2_dice_score : %f model2_class1_dice: %f model2_class2_dice: %f  model2_hd95 : %f model2_class1_hd95 : %f  model2_class2_hd95 : %f' % (
                        iter_num, avg_metric2[:, 0].mean(), avg_metric2[0, 0], avg_metric2[1, 0],\
                        avg_metric2[:, 1].mean(), avg_metric2[0, 1], avg_metric2[1,1]))
                model2.train()
