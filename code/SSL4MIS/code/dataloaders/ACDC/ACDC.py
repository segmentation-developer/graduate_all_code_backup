...
class ACDC(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        ...
        test_path = self._base_dir+'/val.list'
    ...
    def __getitem__(self, idx):
        ...
        image = nib.load(self._base_dir + "/{}/{}".format(image_name[:10], image_name))
        image = image.get_fdata()
        label_name = self.image_list[idx].replace('.nii.gz', '_gt.nii.gz')
        label = nib.load(self._base_dir + "/{}/{}".format(label_name[:10], label_name))
        ...
        # for idx in range(3):
        plt.subplot(2, 1, 1)
        plt.imshow(image[:,:,7:8])
        plt.subplot(2, 1, 2)
        plt.imshow(label[:,:,7:8])
...
class TwoStreamBatchSampler(Sampler):
    ...
    def __iter__(self):
        ...
        #print(primary_batch + secondary_batch)
        #print()
    def __len__(self):
