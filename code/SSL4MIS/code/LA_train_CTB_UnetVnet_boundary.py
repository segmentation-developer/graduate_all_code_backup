...
from LA_val_3D import test_all_case
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            boundary1 = compute_bound(outputs_soft1[:args.labeled_bs,1,:,:,:].detach().cpu().numpy())        #obj channel
            boundary1 = torch.from_numpy(boundary1).float().cuda()

            outputs2 = model2(volume_batch)
            outputs_soft2 = torch.softmax(outputs2, dim=1)
            boundary2 = compute_bound(outputs_soft2[:args.labeled_bs,1,:,:,:].detach().cpu().numpy())
            ...
            #preds = (outputs_soft1[args.labeled_bs:] + outputs_soft2[args.labeled_bs:]) / 2

            ##unsupervised

            '''
            variance1 = dice_loss_class(outputs_soft1[args.labeled_bs:], pseudo_outputs2)
            exp_variance1 = torch.exp(-variance1)

            variance2 = dice_loss_class(outputs_soft2[args.labeled_bs:], pseudo_outputs1)
            ...
            '''
            variance1 = dice_loss_class(outputs_soft1[args.labeled_bs:],preds)      #DiceLoss_class -> target = self._one_hot_encoder(target) 주석 제거
            exp_variance1 = torch.exp(-variance1)

            variance2 = dice_loss_class(outputs_soft2[args.labeled_bs:],preds)
            exp_variance2 = torch.exp(-variance2)

            consistency_dist1 = (outputs_soft1[args.labeled_bs:] - preds) ** 2
            consistency_loss1 = torch.mean(
                consistency_dist1 * exp_variance1) / (torch.mean(exp_variance1) + args.add) + torch.mean(variance1)

            consistency_dist2 = (outputs_soft2[args.labeled_bs:] - preds) ** 2
            consistency_loss2 = torch.mean(
                consistency_dist2 * exp_variance2) / (torch.mean(exp_variance2) + args.add) + torch.mean(variance2)
            '''
            ...
            logging.info(
                'iteration %d : supervised_loss : %f boundary_loss : %f unsupervised_loss : %f  cross_loss1: %f cross_loss2: %f' % (
                    iter_num, supervised_loss.item(), boundary_loss.item(), unsupervised_loss.item(),
                    pseudo_supervision1.item(), pseudo_supervision2.item()))
