...
parser.add_argument('--exp', type=str,
                    default='LA/URPC_entropy', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            variance_aux1 = torch.nan_to_num(variance_aux1, nan=0.0, posinf=1e-10, neginf=-1e-10)
            ...
            variance_aux2 = torch.nan_to_num(variance_aux2, nan=0.0, posinf=1e-10, neginf=-1e-10)
            ...
            variance_aux3 = torch.nan_to_num(variance_aux3, nan=0.0, posinf=1e-10, neginf=-1e-10)
            ...
            variance_aux4 = torch.nan_to_num(variance_aux4, nan=0.0, posinf=1e-10, neginf=-1e-10)
            ...
            consistency_loss_aux1 = torch.mean(
                consistency_dist_aux1 * exp_variance_aux1) / (torch.mean(exp_variance_aux1) + args.add) + torch.mean(variance_aux1)

            consistency_dist_aux2 = (
                preds[args.labeled_bs:] - outputs_aux2_soft[args.labeled_bs:]) ** 2
            consistency_loss_aux2 = torch.mean(
                consistency_dist_aux2 * exp_variance_aux2) / (torch.mean(exp_variance_aux2) + args.add) + torch.mean(variance_aux2)

            consistency_dist_aux3 = (
                preds[args.labeled_bs:] - outputs_aux3_soft[args.labeled_bs:]) ** 2
            consistency_loss_aux3 = torch.mean(
                consistency_dist_aux3 * exp_variance_aux3) / (torch.mean(exp_variance_aux3) + args.add) + torch.mean(variance_aux3)

            consistency_dist_aux4 = (
                preds[args.labeled_bs:] - outputs_aux4_soft[args.labeled_bs:]) ** 2
            consistency_loss_aux4 = torch.mean(
                consistency_dist_aux4 * exp_variance_aux4) / (torch.mean(exp_variance_aux4) + args.add) + torch.mean(variance_aux4)

            consistency_loss = (consistency_loss_aux1 +
                                consistency_loss_aux2 + consistency_loss_aux3 + consistency_loss_aux4) / 4
            loss = supervised_loss + consistency_weight * (consistency_loss + (0.5) * entropyLoss)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            logging.info(
                'iteration %d : loss : %f, supervised_loss: %f, consistency_loss_aux1: %f, consistency_loss_aux2: %f, consistency_loss_aux3: %f, consistency_loss_aux4: %f,\
                    consistency_dist_aux1 : %f, exp_variance_aux1: %f, variance_aux1: %f, variance_aux2: %f, variance_aux3: %f,variance_aux4: %f' %
                (iter_num, loss.item(), supervised_loss.item(),consistency_loss_aux1.item(),consistency_loss_aux2.item(),consistency_loss_aux3.item(), consistency_loss_aux4.item(), \
                    torch.mean(consistency_dist_aux1).item(),torch.mean(exp_variance_aux1).item(), torch.mean(variance_aux1).item(), torch.mean(variance_aux2).item(),torch.mean(variance_aux3).item(),torch.mean(variance_aux4).item()))
