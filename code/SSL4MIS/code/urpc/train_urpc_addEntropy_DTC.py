...
from networks.unet_3D_dv_semi_DTC import unet_3D_dv_semi
from utils.util import compute_sdf
from utils import losses, metrics, ramps
from val_urpc_util import test_all_case
import wandb
wandb.init(project="SSL_LA heart4", config ={
        #"batch size" :16,
        #"max_iterations":5000,
        #"encoder": 'resnet-34',
        #"weights": 'imagenet',
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs_aux1, outputs_aux2, outputs_aux3, outputs_aux4, outputs_tanh  = model(
                volume_batch)
            ...
            with torch.no_grad():
                ...
                ).numpy(), outputs_aux1[:args.labeled_bs, 0, ...].shape)
                gt_dis = torch.from_numpy(gt_dis).float().cuda()

            dis_to_mask = torch.softmax(-1500 * outputs_tanh, dim=1)

            #supervised SDM
            loss_sdf = mse_loss(outputs_tanh[:args.labeled_bs, 0, ...], gt_dis)

            #unsupervised SDM
            #consistency_sdm_loss = torch.mean((dis_to_mask[args.labeled_bs:] - outputs_aux1_soft[args.labeled_bs:]) ** 2)
            ...
            supervised_loss = (loss_sdf+loss_ce_aux1+loss_ce_aux2+loss_ce_aux3+loss_ce_aux4 +
                               loss_dice_aux1+loss_dice_aux2+loss_dice_aux3+loss_dice_aux4)/9
