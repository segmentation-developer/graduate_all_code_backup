...
from networks.utils import UnetDsv3
import wandb
...
wandb.init(project="SSL_LAheart", config={}, reinit=True)
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs1,_, dsv4_1 = model1(volume_batch + noise )
            outputs_soft1 = torch.softmax(outputs1, dim=1)

            noise = torch.clamp(torch.randn_like(volume_batch) * 0.1, -0.2, 0.2)
            outputs2,_, dsv4_2 = model2(volume_batch + noise )
            ...
            if epoch_num == 200:
                ...
                # for idx in range(3):
                plt.subplot(6, 1, 1)
                plt.imshow(label_batch[0][:, :, 70:71].detach().cpu())
                plt.subplot(6, 1, 2)
                plt.imshow(boundary_label[0][:, :, 70:71])
                plt.subplot(6, 1, 3)
                plt.imshow(outputs_soft1[0][0][:, :, 70:71].detach().cpu())
                plt.subplot(6, 1, 4)
                plt.imshow(outputs_soft1[0][1][:, :, 70:71].detach().cpu())
                plt.subplot(6, 1, 5)
                plt.imshow(outputs_soft2[0][0][:, :, 70:71].detach().cpu())
                plt.subplot(6, 1, 6)
                plt.imshow(outputs_soft2[0][1][:, :, 70:71].detach().cpu())
            ...
            ul_enc_KL_loss = torch.sum(kl_distance(
                torch.log(torch.softmax(dsv4_1[args.labeled_bs:], dim=1)),
                torch.softmax(dsv4_2[args.labeled_bs:], dim=1)), dim=1, keepdim=True)
            ul_enc_KL_loss = torch.mean(ul_enc_KL_loss)

            '''
            dsv4 = (dsv4_1[args.labeled_bs:] + dsv4_2[args.labeled_bs:]) / 2
            ul_enc_KL_loss1 = torch.sum(kl_distance(
                torch.log(torch.softmax(dsv4_1[args.labeled_bs:], dim=1)), torch.softmax(dsv4, dim=1)), dim=1,
                keepdim=True)
            ul_enc_KL_loss1 = torch.mean(ul_enc_KL_loss1)

            
            ul_enc_KL_loss2 = torch.sum(kl_distance(
                torch.log(torch.softmax(dsv4_2[args.labeled_bs:], dim=1)), torch.softmax(dsv4, dim=1)), dim=1,
                keepdim=True)
            ul_enc_KL_loss2 = torch.mean(ul_enc_KL_loss2)
            '''
            supervised_loss= loss1 + loss2
            unsupervised_loss =  consistency_weight *( pseudo_supervision1 + pseudo_supervision2 + ul_enc_KL_loss)
            ...
            wandb.log({
                ...
                "cross_loss1" : pseudo_supervision1.item(),
                "cross_loss2" : pseudo_supervision2.item(),
                "consistency_weight":consistency_weight,
                "enc4_KL[2:4]_1": ul_enc_KL_loss.item(),

            })

            logging.info('iteration %d : supervised_loss : %f  unsupervised_loss : %f  cross_loss1: %f cross_loss2: %f'  % (
                iter_num, supervised_loss.item(), unsupervised_loss.item(), pseudo_supervision1.item(), pseudo_supervision2.item()))
