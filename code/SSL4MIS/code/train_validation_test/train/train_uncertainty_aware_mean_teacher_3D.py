...
parser.add_argument('--exp', type=str,
                    default='/train_result/BraTs2019_Mean_Teacher', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # for idx in range(3):
            plt.subplot(8, 1, 1)
            plt.imshow(volume_batch[0][0][:, :, 60:61].detach().cpu().numpy())
            plt.subplot(8, 1, 2)
            plt.imshow(label_batch[0][:, :, 60:61].detach().cpu().numpy())
            plt.subplot(8, 1, 3)
            plt.imshow(volume_batch[1][0][:, :, 60:61].detach().cpu().numpy())
            plt.subplot(8, 1, 4)
            plt.imshow(label_batch[1][:, :, 60:61].detach().cpu().numpy())
            plt.subplot(8, 1, 5)
            plt.imshow(volume_batch[2][0][:, :, 60:61].detach().cpu().numpy())
            plt.subplot(8, 1, 6)
            plt.imshow(label_batch[2][:, :, 60:61].detach().cpu().numpy())
            plt.subplot(8, 1, 7)
            plt.imshow(volume_batch[3][0][:, :, 60:61].detach().cpu().numpy())
            plt.subplot(8, 1, 8)
            plt.imshow(label_batch[3][:, :, 60:61].detach().cpu().numpy())
...
if __name__ == "__main__":
    ...
    snapshot_path = "/data/sohui/BraTS/{}/{}".format(
        args.exp,  args.model)
