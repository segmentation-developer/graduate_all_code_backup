...
parser.add_argument('--exp', type=str,
                    default='LA/CPS_test', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    net1 = unet_3D( in_channels=1, n_classes=num_classes ).cuda()
    #net1 = nn.DataParallel(net1).to(device)

    net2 = unet_3D( in_channels=1, n_classes=num_classes ).cuda()
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss1 = 0.5 * (ce_loss(outputs1[:args.labeled_bs],label_batch[:args.labeled_bs])
                           + dice_loss(outputs_soft1[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1)))
            loss2 = 0.5 * (ce_loss(outputs2[:args.labeled_bs],label_batch[:args.labeled_bs])
            ...
            variance1 = torch.sum(kl_distance(
                torch.log(outputs_soft1[args.labeled_bs:]), outputs_soft2[args.labeled_bs:]), dim=1, keepdim=True)
            exp_variance1 = torch.exp(-variance1)

            #variance2 = torch.sum(kl_distance(
            #    torch.log(outputs_soft2[args.labeled_bs:]),outputs_soft1[args.labeled_bs:]), dim=1, keepdim=True)
            #exp_variance2 = torch.exp(-variance2)

            consistency_dist1 = (
                outputs_soft2[args.labeled_bs:] - outputs_soft1[args.labeled_bs:]) ** 2
            consistency_loss1 = torch.mean(
                consistency_dist1 * exp_variance1) / (torch.mean(exp_variance1) + args.add) + torch.mean(variance1)
            #consistency_dist2 = (
            #    pseudo_outputs1 - outputs_soft2[args.labeled_bs:]) ** 2
            ...
            #model1_loss = loss1 + consistency_weight * consistency_loss1
            #model2_loss = loss2 + consistency_weight * consistency_loss2
            supervised_loss = (loss1 + loss2)
            loss = supervised_loss +  consistency_weight * consistency_loss1
            ...
            writer.add_scalar('loss/super_loss',
                              supervised_loss, iter_num)
            writer.add_scalar('loss/unsuper_loss',
            ...
            wandb.log({
                ...
                "consistency_loss": consistency_loss1.item(),
            ...
            logging.info(
                'iteration %d : super_loss : %f unsuper_loss : %f' % (iter_num, supervised_loss.item(), consistency_loss1.item()))
