...
from code.dataloaders.LA.la_heart import LAHeart, RandomCrop, RandomRotFlip, ToTensor, TwoStreamBatchSampler
from networks.unet_3D_sdf import unet_3D
from networks.vnet_sdf import VNet
#from monai.networks.nets import UNETR
#from networks.vision_transformer import SwinUnet as ViT_seg
from utils import losses, ramps
from code.validation.val_3D import test_all_case
import wandb
...
parser.add_argument('--exp', type=str,
                    default='LA/CTB(CPS)_unet_vnet_URUM', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs1, outputs_tanh1 = model1(volume_batch)
            outputs_soft1 = torch.softmax(outputs1, dim=1)
            dis_to_mask1 = torch.softmax(-1500 * outputs_tanh1, dim=1)

            outputs2, outputs_tanh2 = model2(volume_batch)
            outputs_soft2 = torch.softmax(outputs2, dim=1)
            dis_to_mask2 = torch.softmax(-1500 * outputs_tanh2, dim=1)

            '''
            with torch.no_grad():
                gt_dis = compute_sdf(label_batch[:].cpu(
                ).numpy(), outputs1[:args.labeled_bs, 0, ...].shape)
            ...
            ##supervised
            loss1 = args.alpha * (ce_loss(outputs1[:args.labeled_bs], label_batch[:args.labeled_bs].long()) + dice_loss(
                outputs_soft1[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1)))
            loss2 = args.alpha * (ce_loss(outputs2[:args.labeled_bs], label_batch[:args.labeled_bs].long()) + dice_loss(
                outputs_soft2[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1)))

            ##pseudo label(2,2,112,112,80)
            ...
            mse_loss1 = torch.mean((dis_to_mask1[args.labeled_bs:] - outputs_soft2[args.labeled_bs:]) ** 2)
            ...
            mse_loss2 = torch.mean((dis_to_mask2[args.labeled_bs:] - outputs_soft1[args.labeled_bs:]) ** 2)

            model1_loss = loss1 + consistency_weight * (consistency_loss1 + 0.5 * (mse_loss1))
            model2_loss = loss2 + consistency_weight * (consistency_loss2 + 0.5 * (mse_loss2))

            ## loss 현황을 보기 위한 변수 (loss backward와는 상관 없음)
            supervised_loss = loss1.item()+loss2.item()
            unsupervised_loss = (consistency_loss1.item() + 0.5 * (mse_loss1.item())) + (consistency_loss2.item() + 0.5 * (mse_loss2.item()))
            ################################################################
            ...
            logging.info('iteration %d : supervised_loss : %f unsupervised_loss : %f' % (
                iter_num, supervised_loss, unsupervised_loss))
