...
from networks.unet_3D_noise_RandomCrop import unet_3D
...
def train(args, snapshot_path):
    ...
    def compute_sdf(img_gt):  # label_batch[:labeled_bs, 0, ...].shape
        """
        ...
        for b in range(img_gt[0]):  # batch size=2
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs1,aux_outputs1= model1(volume_batch)
            outputs_soft1 = torch.softmax(outputs1, dim=1)
            aux_outputs_soft1 = torch.softmax(aux_outputs1, dim=1)


            outputs2,aux_outputs2 = model2(volume_batch)
            outputs_soft2 = torch.softmax(outputs2, dim=1)
            aux_outputs_soft2 = torch.softmax(aux_outputs2, dim=1)
            ...
            #boundary_loss1 = mse_loss(outputs_soft1[:args.labeled_bs],label_batch[:args.labeled_bs].unsqueeze(1))
            #boundary_loss2 = mse_loss(outputs_soft2[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1))
            ...
            crop_mseLoss = 1 * mse_loss(aux_outputs_soft1[args.labeled_bs:],aux_outputs_soft2[args.labeled_bs:])

            supervised_loss= loss1 + loss2
            unsupervised_loss =  consistency_weight *(pseudo_supervision1 +pseudo_supervision2 +mseLoss1 + mseLoss2 + crop_mseLoss)
            ...
            logging.info('iteration %d : supervised_loss : %f unsupervised_loss : %f mseLoss1 : %f mseLoss2 : %f crop_mseLoss : %f'  % (
                iter_num, supervised_loss.item(), unsupervised_loss.item(),mseLoss1.item(),mseLoss2.item(),crop_mseLoss.item()))
