...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            dsv5_kl_loss = torch.sum(kl_distance( torch.log(dsv5_F_soft[args.labeled_bs:]),dsv5_S_soft[args.labeled_bs:]), dim=1, keepdim=True)
            dsv5_kl_loss = torch.mean(dsv5_kl_loss)

            supervised_loss= (loss1 + loss2)/2
            unsupervised_loss =  consistency_weight *((consistency_loss_aux1+ consistency_loss_aux2+ consistency_loss_aux3+consistency_loss_aux4+dsv5_kl_loss)/5)
            ...
            wandb.log({
                ...
                #"dsv5_kl_loss": dsv5_kl_loss.item(),
