...
def train(args, snapshot_path):
    ...
    def info_nce_loss(features1,features2):
        ...
        ch = features1.shape[1]
        width = features1.shape[2]
        features1 = features1.view(batch_size, ch, -1).permute(0, 2, 1)            # batch_size, ch, hwd -> batch_size, hwd,ch: ,245,256
        features2 = features2.view(batch_size, ch, -1).permute(0, 2, 1)
        features1 = F.normalize(features1, dim=-1, p=1)
        features2 = F.normalize(features2, dim=-1, p=1)
        features2 = features2.detach()

        similarity_matrix = torch.bmm(features1.reshape(-1, batch_size, ch), features2.reshape(-1, ch, batch_size))
        mask = torch.zeros([1003520,2,2])
        mask[:, 0, 0] = 1
        mask[:, 1, 1] = 1

        # pos logit
        positives = similarity_matrix[mask==1].view(similarity_matrix.shape[0], -1)

        # neg logit
        negatives = similarity_matrix[mask==0].view(similarity_matrix.shape[0], -1)


        out = torch.cat((positives, negatives), dim=1) / args.temperature


        loss = cross_entropy_loss(out, torch.zeros(out.size(0), dtype=torch.long).cuda())
    ...
    '''
    def info_nce_loss(features1,features2):
        labels = torch.cat([torch.arange(4) for i in range(1)], dim=0)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            dsv1_F, dsv2_F, dsv3_F, dsv4_F, dsv5_F = model1(volume_batch + noise )
            ...
            dsv5_F_soft = torch.softmax(dsv5_F, dim=1)

            noise = torch.clamp(torch.randn_like(volume_batch) * 0.1, -0.2, 0.2)
            dsv1_S, dsv2_S, dsv3_S, dsv4_S, dsv5_S = model2(volume_batch + noise )
            ...
            dsv5_S_soft = torch.softmax(dsv5_S, dim=1)



            label_batch = label_batch.contiguous().view(4, -1)
            this_y = label_batch[0]
            this_classes = torch.unique(this_y)

            feats = dsv1_S.permute(0, 2, 3 ,4, 1)
            feats = feats.contiguous().view(feats.shape[0], -1, feats.shape[-1])
            this_classes = [x for x in this_classes if x != -1]
            ...
            #dsv5_kl_loss = torch.sum(kl_distance( torch.log(dsv5_F_soft),dsv5_S_soft), dim=1, keepdim=True)
            #dsv5_kl_loss = torch.mean(dsv5_kl_loss)

            dsv1_contrastive_loss = info_nce_loss(features1 = dsv1_F_soft[args.labeled_bs:], features2 = dsv1_S_soft[args.labeled_bs:])



            supervised_loss= (loss1 + loss2)/2
            unsupervised_loss =  consistency_weight *((consistency_loss_aux1+ consistency_loss_aux2+ consistency_loss_aux3+consistency_loss_aux4 + dsv1_contrastive_loss)/5)
            ...
            wandb.log({
                ...
                "dsv1_contrastive_loss": dsv1_contrastive_loss.item(),

            })

            logging.info('iteration %d : supervised_loss : %f  unsupervised_loss : %f dsv1_contrastive_loss: %f'  % (
                iter_num, supervised_loss.item(), unsupervised_loss.item(),  dsv1_contrastive_loss.item()))
