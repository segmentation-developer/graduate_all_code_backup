...
from networks.unet_3D_32 import unet_3D
...
parser.add_argument('--root_path', type=str,
                    default='/data/sohui/HV/Task08_HepaticVessel_pre', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='HV_result/Unet2_SL_imgCrop_slidingW144_Unet', help='experiment_name')
...
parser.add_argument('--patch_size', type=list,  default=[112,112,64],
...
parser.add_argument('--gpu', type=str,  default='1,2,6', help='GPU to use')
...
wandb.init(project="SSL_HV", config={}, reinit=True)
...
def train(args, snapshot_path):
    ...
    def create_model(ema=False):
        ...
        #model = VNet(n_channels=1, n_classes=num_classes, n_filters=16, normalization='batchnorm',has_dropout=True)    # 기존 vnet
        '''
        model = UNet(
            spatial_dims=3,
            in_channels=1,
    ...
    train_transforms = Compose(
        [
            ...
            #CenterSpatialCropd(keys=['image', 'label'], roi_size=(352,352,64)),
            #SpatialPadd(keys=["image", "label"], spatial_size=(352,352, 64), mode="constant"),
            #RandCropByPosNegLabeld(
            #    keys=["image", "label"],
            #    label_key="label",
    ...
    train_files, val_files = datalist, datalist[-60:]   # 242,60
    ...
    SL_trainloader = DataLoader(db_train_SL, batch_size=126,shuffle=False,
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(SL_trainloader):       #0,1 : SL_trainloader(bs:1), UL_trainloader(bs:1)
            ...
            dsv1_F = sliding_window_inference(
                volume_batch, roi_size, sw_batch_size, model1)
            ...
            # for idx in range(3):
            plt.subplot(3, 1, 1)
            plt.imshow(volume_batch[0][0][:, :, 30:31].detach().cpu())
            plt.subplot(3, 1, 2)
            plt.imshow(dsv1_F[0][0][:, :, 30:31].detach().cpu())
            plt.subplot(3, 1, 3)
            plt.imshow(label_batch[0][0][:, :, 30:31].detach().cpu())
            plt.tight_layout()
            plt.show()
            print()
            '''
