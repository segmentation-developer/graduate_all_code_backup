...
class Decoder(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False):
        ...
        # deep supervision
        self.dsv4 = UnetDsv3(
            in_size=n_filters * 8, out_size=n_classes, scale_factor=8)
        self.dsv3 = UnetDsv3(
            in_size=n_filters * 4, out_size=n_classes, scale_factor=4)
        self.dsv2 = UnetDsv3(
            in_size=n_filters * 2, out_size=n_classes, scale_factor=2)
        self.dsv1 = nn.Conv3d(
            in_channels=n_filters, out_channels=n_classes, kernel_size=1)
    ...
    def forward(self, features):
        ...
        x6 = self.block_six(torch.cat([x5_up, x4], 1))
        x6 = self.pointwise_six(x6)
        x6_up = self.block_six_up(x6)

        x7 = self.block_seven(torch.cat([x6_up, x3], 1))
        x7 = self.pointwise_seven(x7)
        x7_up = self.block_seven_up(x7)

        x8 = self.block_eight(torch.cat([x7_up, x2], 1))
        x8 = self.pointwise_eight(x8)
        x8_up = self.block_eight_up(x8)

        x9 = self.block_nine(torch.cat([x8_up, x1], 1))
...
class FeatureNoise(nn.Module):
    ...
    def feature_based_noise(self, x):
        ...
        x_noise = x.mul(noise_vector) + x           #x.mul(noise_vector): always same
        return x_noise
...
class VNet(nn.Module):
    ...
    def forward(self, input, turnoff_drop=False):
        ...
        dsv1, dsv2, dsv3, dsv4 = self.main_decoder(features)
        aux1_feature = [FeatureNoise()(i) for i in features]
        aux_dsv1, aux_dsv2, aux_dsv3, aux_dsv4 = self.aux_decoder1(aux1_feature)

        if turnoff_drop:
            self.has_dropout = has_dropout
        return dsv1, dsv2, dsv3, dsv4, aux_dsv1, aux_dsv2, aux_dsv3, aux_dsv4
