...
class CenterCrop(object):
    ...
    def __call__(self, sample):
        ...
        (b, ch, w, h, d) = image.shape

        w1 = int(round((w - self.output_size[2]) / 2.))
        h1 = int(round((h - self.output_size[3]) / 2.))
        #d1 = int(round((d - self.output_size[4]) / 2.))


        image = image[0:b, 0:ch, w1:w1 + self.output_size[2], h1:h1 + self.output_size[3], 0:d]
...
class unet_3D(nn.Module):
    ...
    def forward(self, inputs):
        ...
        Center = CenterCrop([4, 2, 90, 90, 80])
        #aux1_out = FeatureNoise()(final)
        aux1_out = Center(final)

        return final, aux1_out
