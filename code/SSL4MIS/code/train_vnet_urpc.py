...
from dataloaders.LA.la_heart_modi import LAHeart, RandomCrop, RandomRotFlip, ToTensor, TwoStreamBatchSampler
from networks.vnet_concat_dv_semi import VNet
#from networks.unet_3D_dv_semi import unet_3D_dv_semi
from utils import losses, ramps
from val_urpc_util_modi import test_all_case
import wandb
...
parser.add_argument('--exp', type=str,
                    default='LA/URPC_vnet_test', help='experiment_name')
...
parser.add_argument('--fold', type=str,  default='fold3')
...
def train(args, snapshot_path, fold, train_idx, val_idx):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            #variance_aux1 = torch.sum:[2,1,112,112,80]
            #exp_variance_aux1 :[2,1,112,112,80]
            #kl_dis : [2,2,112,112,80]
            #torch.mean(variance_aux1) :1
    ...
    for fold , (train_idx, val_idx) in enumerate(kf.split(image_list)):
        ...
        if args.fold == 'front' :
            if fold < 3 :
                ...
                print('front fold training finish!')
                break
        if args.fold == 'behind':
            if fold >= 3 :
                wandb.init(project="SSL_LA heart_kfold", config={}, reinit=True)
