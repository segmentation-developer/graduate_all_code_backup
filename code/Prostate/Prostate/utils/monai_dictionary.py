...
from Prostate.utils.monai_array import LabelToContour
...
class Activationsd(MapTransform):
    ...
    def __init__(
        ...
        """
        Args:
            keys: keys of the corresponding items to denseUnet_3D output and label.
...
class Invertd(MapTransform):
    ...
    A typical usage is to apply the inverse of the preprocessing on input ``image`` to the denseUnet_3D ``pred``.
...
class SaveClassificationd(MapTransform):
    ...
    def __init__(
        ...
        """
        Args:
            keys: keys of the corresponding items to denseUnet_3D output, this transform only supports 1 key.
                See also: :py:class:`monai.transforms.compose.MapTransform`
