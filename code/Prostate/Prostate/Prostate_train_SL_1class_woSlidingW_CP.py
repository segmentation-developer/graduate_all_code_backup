...
from Prostate.networks.vnet_aggM import VNet
...
def train(args, snapshot_path):
    ...
    def construct_ideal_affinity_matrix(label): #2,1,176,176,176
        one_hot_labels = F.one_hot(label.to(torch.int64), args.num_classes) # one hot encoding으로 바꿔줌 -> 2,1,176,176,176,2
        one_hot_labels = one_hot_labels.view(
            one_hot_labels.size(0), -1, args.num_classes).float()       # bs, hwd, ch :2,5451776,2

        ideal_affinity_matrix = torch.bmm(one_hot_labels,                        # ideal affinity map을 만드는 과정에서 out of mem error
                                          one_hot_labels.permute(0, 2, 1))       # bs, hwd, ch * bs, ch, hwd = bs, hwd, hwd
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(SL_trainloader):       #0,1 : SL_trainloader(bs:1), UL_trainloader(bs:1)
            ...
            one_hot_label_batch = construct_ideal_affinity_matrix(label_batch)

            #noise = torch.clamp(torch.randn_like(volume_batch) * 0.1, -0.2, 0.2)
            #volume_batch = volume_batch + noise

            output = model(volume_batch)        # 2,1,192,192,176 -> 2,2,192,192,176
            output_soft = torch.softmax(output, dim=1)

            ##supervised :dice CE
            loss = (ce_loss(output, label_batch.squeeze(1).long())
                    + dice_loss(output_soft, label_batch))
