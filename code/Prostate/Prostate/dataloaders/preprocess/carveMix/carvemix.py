...
def get_distance(f, spacing):           #label,spacing
...
def generate_new_sample(image_a, image_b, label_a, label_b):
    ...
    # for idx in range(3):
    plt.subplot(2, 1, 1)
    plt.imshow(label_b[:, :, 100:101],cmap ='gray')
    plt.subplot(2, 1, 2)
    plt.imshow(dis_array[:, :, 100:101],cmap ='gray')
...
if __name__ == '__main__':
    ...
    parse.add_argument("--imagesTr_path", "-imgTr", default="/home/psh/data/Brats19_numclass_2/data/BraTs2019_nii/images", type=str,
                       help="Path to raw images")
    parse.add_argument("--labelsTr_path", "-labelTr", default="/home/psh/data/Brats19_numclass_2/data/BraTs2019_nii/labels", type=str,
                       help="Path to raw labels")
    parse.add_argument("--mask_check_path", "-mask", default="/home/psh/data/Brats19_numclass_2/data/BraTs2019_nii/carvemix", type=str,
                       help="Path to save masks generated by CarveMix")
    parse.add_argument("--mixid_csv_path", "-csv", default="/home/psh/data/Brats19_numclass_2/data/BraTs2019_nii/carvemix/CarveMixID.csv", type=str,
    ...
    for i in tqdm(range(opt.generate_number), desc="mixing:"):
        ...
        image_a = os.path.join(opt.imagesTr_path, Cases[rand_index_a]+'.nii.gz' )
        label_a = os.path.join(opt.labelsTr_path, Cases[rand_index_a]+'.nii.gz')
        image_b = os.path.join(opt.imagesTr_path, Cases[rand_index_b]+'.nii.gz')
        ...
        s = str(i)
        sitk.WriteImage(new_target, os.path.join(opt.mask_check_path,'/images' + 'CarveMix_' + s + '.nii.gz'))
        sitk.WriteImage(new_label, os.path.join(opt.mask_check_path, '/labels'+ 'CarveMix_' + s + '.nii.gz'))
        if i % 100 == 0:
            sitk.WriteImage(mask, os.path.join(opt.mask_check_path, '/mask' + 'CarveMix_mask' + s + '.nii.gz'))
