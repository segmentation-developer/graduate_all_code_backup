...
from Prostate.networks import VNet
...
parser.add_argument('--root_path', type=str,
                    default='/data/sohui/Prostate/data', help='Name of Experiment')
...
class AggregationModule(nn.Module):
    ...
    def __init__(self,
        ...
        self.t1 = nn.Sequential(
            nn.Conv3d(out_channels,out_channels,kernel_size=(kernel_size, 1,kernel_size),padding= (padding, 0,padding)),
            nn.BatchNorm3d(out_channels),
            nn.ReLU(inplace=True))

        self.t2 = nn.Sequential(
            nn.Conv3d(out_channels,out_channels,kernel_size=(1, kernel_size,kernel_size),padding=(0, padding,padding)),
            nn.BatchNorm3d(out_channels),
            nn.ReLU(inplace=True))
...
def train(args, snapshot_path):
    ...
    def create_model(ema=False):
        ...
        aggM = AggregationModule(in_channels=args.num_classes, out_channels=args.num_classes,
                                 kernel_size=args.aggKernel)
        if ema:
            for param in model.parameters():
                param.detach_()
        return model, aggM

    model,aggM = create_model()
    if len(args.gpu.split(',')) > 1:
        model = nn.DataParallel(model).to(device)
        aggM = nn.DataParallel(aggM).to(device)
    else :
        model = model.cuda()
        aggM = aggM.cuda()
    ...
    train_transforms = Compose(
        [
            ...
            CenterSpatialCropd(keys=['image', 'label'], roi_size=(192,192,176)),
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(SL_trainloader):       #0,1 : SL_trainloader(bs:1), UL_trainloader(bs:1)
            ...
            # for idx in range(3):
            plt.subplot(3, 1, 1)
            plt.imshow(output_[0][0][:, :,100:101].detach().cpu().numpy(), cmap='gray')
            plt.subplot(3, 1, 2)
            plt.imshow(output[0][0][:, :,100:101].detach().cpu().numpy(), cmap='gray')
